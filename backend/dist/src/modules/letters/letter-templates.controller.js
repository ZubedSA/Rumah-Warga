"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.LetterTemplatesController = void 0;
const common_1 = require("@nestjs/common");
const letter_templates_service_1 = require("./letter-templates.service");
const create_letter_template_dto_1 = require("./dto/create-letter-template.dto");
const jwt_auth_guard_1 = require("../../common/guards/jwt-auth.guard");
const roles_guard_1 = require("../../common/guards/roles.guard");
const tenant_decorator_1 = require("../../common/decorators/tenant.decorator");
let LetterTemplatesController = class LetterTemplatesController {
    letterTemplatesService;
    constructor(letterTemplatesService) {
        this.letterTemplatesService = letterTemplatesService;
    }
    create(tenantId, createLetterTemplateDto) {
        return this.letterTemplatesService.create(tenantId, createLetterTemplateDto);
    }
    findAll(tenantId) {
        return this.letterTemplatesService.findAll(tenantId);
    }
    findOne(tenantId, id) {
        return this.letterTemplatesService.findOne(tenantId, id);
    }
    update(tenantId, id, updateLetterTemplateDto) {
        return this.letterTemplatesService.update(tenantId, id, updateLetterTemplateDto);
    }
    remove(tenantId, id) {
        return this.letterTemplatesService.remove(tenantId, id);
    }
};
exports.LetterTemplatesController = LetterTemplatesController;
__decorate([
    (0, common_1.Post)(),
    __param(0, (0, tenant_decorator_1.TenantId)()),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, create_letter_template_dto_1.CreateLetterTemplateDto]),
    __metadata("design:returntype", void 0)
], LetterTemplatesController.prototype, "create", null);
__decorate([
    (0, common_1.Get)(),
    __param(0, (0, tenant_decorator_1.TenantId)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], LetterTemplatesController.prototype, "findAll", null);
__decorate([
    (0, common_1.Get)(':id'),
    __param(0, (0, tenant_decorator_1.TenantId)()),
    __param(1, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", void 0)
], LetterTemplatesController.prototype, "findOne", null);
__decorate([
    (0, common_1.Patch)(':id'),
    __param(0, (0, tenant_decorator_1.TenantId)()),
    __param(1, (0, common_1.Param)('id')),
    __param(2, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String, Object]),
    __metadata("design:returntype", void 0)
], LetterTemplatesController.prototype, "update", null);
__decorate([
    (0, common_1.Delete)(':id'),
    __param(0, (0, tenant_decorator_1.TenantId)()),
    __param(1, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", void 0)
], LetterTemplatesController.prototype, "remove", null);
exports.LetterTemplatesController = LetterTemplatesController = __decorate([
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard, roles_guard_1.RolesGuard),
    (0, common_1.Controller)('letter-templates'),
    __metadata("design:paramtypes", [letter_templates_service_1.LetterTemplatesService])
], LetterTemplatesController);
//# sourceMappingURL=letter-templates.controller.js.map