...
export default function ExpenseTab() {
  ...
  const fetchTransactions = async () => {
    try {
      setIsLoading(true);
      const res = await apiClient.get('/finance/transactions?type=EXPENSE', {
  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-100 shadow-[0_4px_24px_rgb(0,0,0,0.03)] overflow-hidden">
      <div className="p-6 md:p-8 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-black text-slate-800 tracking-tight">Kas Keluar</h2>
          <p className="text-sm font-medium text-slate-500 mt-1">Kelola seluruh pengeluaran kas RT/RW</p>
        </div>
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
          ...
          <button 
            ...
            className="flex items-center justify-center gap-2 bg-rose-500 text-white px-5 py-2.5 rounded-full text-sm font-bold hover:bg-rose-600 shadow-sm transition-colors w-full sm:w-auto"
          >
            ...
            {transactions.length === 0 ? (
              ...
              transactions.map((tx) => (
                <tr key={tx.id} className="hover:bg-slate-50 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-sm font-black text-rose-600 text-right">-{formatCurrency(tx.amount)}</td>
